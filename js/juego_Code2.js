...
var instrucciones = ["Utilizar las flechas del teclado para mover las piezas", 
                      "Ordenar las piezas hasta alcanzar la imángen del objetivo"];
// Arreglo para ir guardando los movimientos que se vayan realizando
var movimientos = [];

var i, j; //VARIABLES GLOBALES QUE RECORREN LOS ARREGLOS - AGREGADO POR MI
...
function chequearSiGano() {
  ...
  var orden = 1;  //INICIA CON EL VALOR QUE TENDRIA QUE TENER LA POSICION grilla[0][0]

  for (i = 0; i < grilla.length; i++)
    for (j = 0; j < grilla.length; j++){

      if (grilla[i][j] != orden){   //COMPARA SI CADA ELEMENTO DE LA GRILLA ESTÁ ORDENADA
...
function mostrarCartelGanador() {
    var resp;
    alert("FELICITACIONES !!!!\nORDENASTE TODAS LAS PIEZAS EN "+movimientos.length+" MOVIMIENTOS\nERES UN BUEN JUGADOR!!!");

    resp = prompt("QUIERES JUEGAR OTRA VEZ ? (S/N");

    if (resp === "s" || resp === "S"){ //PREGUNTA AL USUARIO SI QUIERE JUGAR DE NUEVO
      
      movimientos = [];             //Inicializa movimientos para el nuevo juego
      mezclarPiezas(30);            // MEZCLAS NUEVAMENTE LAS PIEZAS
    }
    else
      window.close();               //CIERRA LA VENTANA SI EL USUARIO NO QUIERE JUGAR OTRA VEZ
...
function intercambiarPosicionesGrilla(filaPos1, columnaPos1, filaPos2, columnaPos2) {
    ...
    var posicionAuxilar; //ES UNA VARIABLE AUXILIAR QUE ALMACENA UNA POSICION DE LA GRILLA A INTERCAMBIAR

    posicionAuxilar = grilla[filaPos1][columnaPos1];
    grilla[filaPos1][columnaPos1] = grilla[filaPos2][columnaPos2];
    grilla[filaPos2][columnaPos2] = posicionAuxilar;
    //COMPLETADO
}
...
function moverEnDireccion(direccion) {
  ...
  if (direccion === codigosDireccion.ABAJO) {
    //nuevaFilaPiezaVacia = filaVacia - 1; //NO FUNCIONA CON EL CODIGO ORIGINAL
    nuevaFilaPiezaVacia = filaVacia + 1; // CORREGIDO MIO
  ...
  // Mueve pieza hacia la derecha, reemplazandola con la blanca
    ...
    nuevaColumnaPiezaVacia = columnaVacia+1; //CORREGIDO MIO
    //COMPLETADO
  }
...
function mezclarPiezas(veces) {
  ...
  movimientos.pop();  // AGRAGADO MIO - Evita que guarde como movimientos los que son generados por la computadora al mezclar.
